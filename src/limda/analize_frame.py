...
from .neighbor import get_edge_idx_using_cython
from .neighbor import count_molecules_using_cython
from .neighbor import count_bonds_using_cython
from .neighbor import count_coord_numbers_using_cython
...
class AnalizeFrame(
    ...
    def __init__(self):
        pass
#---------------------------------------------------------------------------------------------------------    
    def set_neighbor_info(self,
        mode : str = "",
        cut_off : float = None,
        bond_length : list[list[float]] = []
        )->[list[list[float]], float]:
        assert mode == 'B' or mode == 'C', "Please configure mode"
        ...
        if mode == 'B':
            ...
            for bond_list in bond_length:
                assert len(bond_list) == atom_type_num, "Incorrect format of bond length"
        elif mode == 'C':
            if cut_off is None:
                if "cut_off" in self.limda_default:
                    cut_off = self.limda_default["cut_off"]
            bond_length = [[cut_off for _ in range(atom_type_num)] for __ in range(atom_type_num)]

        mesh_length = max(list(map(lambda x: max(x), bond_length))) + 0.01 # cut_off(bond_length) + margin
        if mesh_length*3 > min(self.cell):
            mesh_length = min(self.cell)/3

        return bond_length, mesh_length
#---------------------------------------------------------------------------------------------------------
    def Get_neighbor_list(
        ...
        """
        """
        bond_length, mesh_length = self.set_neighbor_info(mode, cut_off, bond_length)
        neighbor_list = get_neighbor_list_using_cython(atoms_type = self.atoms["type"],
...
#---------------------------------------------------------------------------------------------------------    
    def Get_edge_idx(
        ...
        bond_length, mesh_length = self.set_neighbor_info(mode, cut_off, bond_length)
        edge_idx = get_edge_idx_using_cython(atoms_type = self.atoms["type"],
                                                       atoms_pos = [self.atoms["x"], self.atoms["y"], self.atoms["z"]],
                                                       mesh_length = mesh_length,
                                                       atom_num = len(self),
                                                       bond_length = bond_length,
...
#---------------------------------------------------------------------------------------------------------
    def Count_molecules(
        ...
        bond_length, mesh_length = self.set_neighbor_info(mode, cut_off, bond_length)
        molecules_list = count_molecules_using_cython(atoms_type = self.atoms["type"],
                                                       atoms_pos = [self.atoms["x"], self.atoms["y"], self.atoms["z"]],
                                                       mesh_length = mesh_length,
                                                       atom_num = len(self),
                                                       bond_length = bond_length,
...
#---------------------------------------------------------------------------------------------------------
    def Count_bonds(
        ...
        bond_length, mesh_length = self.set_neighbor_info(mode, cut_off, bond_length)
        bonds_list = count_bonds_using_cython(atoms_type = self.atoms["type"],
                                                       atoms_pos = [self.atoms["x"], self.atoms["y"], self.atoms["z"]],
                                                       mesh_length = mesh_length,
                                                       atom_num = len(self),
                                                       bond_length = bond_length,
...
#---------------------------------------------------------------------------------------------------------   
    def Count_coord_numbers(
        ...
        bond_length, mesh_length = self.set_neighbor_info(mode, cut_off, bond_length)
        coord_numbers_list = count_coord_numbers_using_cython(atoms_type = self.atoms["type"],
                                                       atoms_pos = [self.atoms["x"], self.atoms["y"], self.atoms["z"]],
                                                       mesh_length = mesh_length,
                                                       atom_num = len(self),
                                                       bond_length = bond_length,
